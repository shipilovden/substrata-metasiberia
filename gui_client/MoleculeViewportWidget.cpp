...
void MoleculeViewportWidget::setSpinEnabled(bool enabled, float speed) { spin_timer->setProperty("step", std::max(0.001f, speed * 0.005f)); if(enabled) spin_timer->start(16); else spin_timer->stop(); }
...
bool MoleculeViewportWidget::handleSceneRay(const Vec4f& origin4, const Vec4f& dir4, bool show_context_menu, bool additive, const QPoint& global_pos)
{
	...
	QMenu menu(this);if(atom_hit>=0){QAction*title=menu.addAction(atomDetails(atom_hit));title->setEnabled(false);menu.addSeparator();QAction*label=menu.addAction(QStringLiteral("Show/hide label"));QAction*measure=menu.addAction(QStringLiteral("Start distance measurement"));QAction*all=menu.addAction(QStringLiteral("Select all %1 atoms").arg(atoms[atom_hit].element));QAction*periodic=menu.addAction(QStringLiteral("Open in periodic table"));QAction*chosen=menu.exec(global_pos);if(chosen==label&&actionRequested)actionRequested(QStringLiteral("toggle_labels"),atom_hit);else if(chosen==measure)startMeasurement(QStringLiteral("distance"));else if(chosen==all)selectElement(atoms[atom_hit].element);else if(chosen==periodic)openPeriodicTable(atoms[atom_hit].element);}
	else if(bond_hit>=0){QAction*title=menu.addAction(bondDetails(bond_hit));title->setEnabled(false);menu.addSeparator();QAction*angle=menu.addAction(QStringLiteral("Start angle measurement"));QAction*torsion=menu.addAction(QStringLiteral("Start torsion measurement"));QAction*chosen=menu.exec(global_pos);if(chosen==angle)startMeasurement(QStringLiteral("angle"));else if(chosen==torsion)startMeasurement(QStringLiteral("torsion"));}
	return true;
}
...
void MoleculeViewportWidget::paintEvent(QPaintEvent*)
{
	...
	for(int i:order){if(!options.show_hydrogen&&atoms[i].element==QStringLiteral("H"))continue;QColor c=QString::fromStdString(options.colour_scheme).compare(QStringLiteral("CPK"),Qt::CaseInsensitive)==0?cpkColour(atoms[i].element):QColor((int)(options.display_colour.r*255),(int)(options.display_colour.g*255),(int)(options.display_colour.b*255));const bool sel=selected_atoms.contains(i);p.setBrush(c);p.setPen(QPen(sel?QColor(255,215,65):c.lighter(150),sel?4:1));p.drawEllipse(pr[i].point,pr[i].radius,pr[i].radius);if(labels_visible&&labels++<options.label_max_count){QFont f=p.font();const double label_scale=std::max(0.35,std::min(2.25,(double)options.label_scale));f.setPointSizeF(std::max(7.0,9.0*label_scale));f.setBold(true);p.setFont(f);p.setPen(QColor((int)(options.label_colour.r*255),(int)(options.label_colour.g*255),(int)(options.label_colour.b*255)));p.drawText(pr[i].point+QPointF(pr[i].radius+3,-pr[i].radius-2),atomLabel(atoms[i]));}}
...
void MoleculeViewportWidget::contextMenuEvent(QContextMenuEvent*e)
{
	...
	if(atom>=0){QAction*title=menu.addAction(atomDetails(atom));title->setEnabled(false);menu.addSeparator();QAction*toggle=menu.addAction(QStringLiteral("Show/hide label"));QAction*center=menu.addAction(QStringLiteral("Center atom"));QAction*measure=menu.addAction(QStringLiteral("Start distance measurement"));QAction*all=menu.addAction(QStringLiteral("Select all %1 atoms").arg(atoms[atom].element));QAction*periodic=menu.addAction(QStringLiteral("Open in periodic table"));QAction*chosen=menu.exec(e->globalPos());if(chosen==toggle){if(actionRequested)actionRequested(QStringLiteral("toggle_labels"),atom);}else if(chosen==center){pan+=QPointF(width()*0.5,height()*0.5)-pr[atom].point;zoom=std::max(zoom,2.f);update();}else if(chosen==measure)startMeasurement(QStringLiteral("distance"));else if(chosen==all)selectElement(atoms[atom].element);else if(chosen==periodic)openPeriodicTable(atoms[atom].element);return;}
	if(bond>=0){QAction*title=menu.addAction(bondDetails(bond));title->setEnabled(false);menu.addSeparator();QAction*angle=menu.addAction(QStringLiteral("Start angle measurement"));QAction*torsion=menu.addAction(QStringLiteral("Start torsion measurement"));QAction*chosen=menu.exec(e->globalPos());if(chosen==angle)startMeasurement(QStringLiteral("angle"));else if(chosen==torsion)startMeasurement(QStringLiteral("torsion"));return;}
	QAction*card=menu.addAction(QStringLiteral("Open molecule card"));QAction*pubchem=menu.addAction(QStringLiteral("Open PubChem page"));QAction*properties=menu.addAction(QStringLiteral("Properties"));QAction*classifications=menu.addAction(QStringLiteral("Classifications"));QAction*images=menu.addAction(QStringLiteral("Images"));QAction*favorite=menu.addAction(QStringLiteral("Add/remove favorite"));menu.addSeparator();QAction*copycid=menu.addAction(QStringLiteral("Copy CID"));QAction*copysmiles=menu.addAction(QStringLiteral("Copy SMILES"));QAction*copyinchi=menu.addAction(QStringLiteral("Copy InChI"));menu.addSeparator();QAction*exporta=menu.addAction(QStringLiteral("Export"));QAction*refresh=menu.addAction(QStringLiteral("Refresh"));QAction*deletea=menu.addAction(QStringLiteral("Delete molecule"));QAction*chosen=menu.exec(e->globalPos());

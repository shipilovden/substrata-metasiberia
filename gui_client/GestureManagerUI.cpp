...
GestureManagerUI* GestureManagerUI::s_active_instance = nullptr;
...
GestureManagerUI* GestureManagerUI::getActiveInstance()
{
...
void GestureManagerUI::addGestureFromSelectedPath(const std::string& selected_path)
{
	...
	try
	{
		...
		{
			// Take the animation name from the GLB filename.
	...
	if(gui_client)
	{
		...
		if(button)
		{
			if(button == add_gesture_button.ptr())
			{
				...
				addGestureFromSelectedPath(selected_path);
			}

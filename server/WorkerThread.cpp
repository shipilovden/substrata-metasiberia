...
void WorkerThread::handleScreenshotBotConnection()
{
	...
	try
	{
		while(!should_quit)
		{
			...
			std::string screenshot_world_name; // Only used for map tiles.
			...
			if(screenshot.nonNull()) // If there is a screenshot to take:
			{
				...
				else if(screenshot->screenshot_type == Screenshot::ScreenshotType_MapTile)
				{
					...
					// Include world name so the screenshot bot can switch worlds on the GUI slave.
					socket->writeStringLengthFirst(screenshot_world_name);
				...
				if(result == Protocol::ScreenShotSucceeded)
				{
...

					{
						...
						{
							MapTileInfo& map_tile_info = server->world_state->getMapTileInfoForWorld(screenshot_world_name, lock);
				...
				{
					conPrint("Screenshot bot reported screenshot taking failed.");

					// If screenshot bot failed, keep the tile request in the queue so it can be retried later.
					// (The request stays in ScreenshotState_notdone.)
					if(screenshot->is_map_tile)
					{
						WorldStateLock lock(server->world_state->mutex);
						PendingMapTileScreenshot pending;
						pending.world_name = screenshot_world_name;
						pending.tile_coords = Vec3<int>(screenshot->tile_x, screenshot->tile_y, screenshot->tile_z);
						server->world_state->pending_map_tile_screenshots.push_back(pending);
					}
...
static bool isSummonedVehicleObject(const WorldObject& ob)
{
...
{
	return isSummonedVehicleObject(ob);
}
...
static bool clientCanControlVehicleObject(const WorldObject& ob, const UID& client_avatar_uid, ServerWorldState& world_state, WorldStateLock& lock)
{
	...
	if(av_res == world_state.getAvatars(lock).end())
		return false;

	const Avatar* avatar = av_res->second.ptr();
	return avatar && (avatar->vehicle_inside_uid == ob.uid);
}
			...
			while(!should_quit) // write to / read from socket loop
			{
...
#endif
				{
					...
					case Protocol::QueryMapTiles:
						{
							...
							std::vector<std::string> result_URLs(num_tiles);
							{
								...
								const std::string world_name = cur_world_state.nonNull() ? cur_world_state->details.name : std::string();
								MapTileInfo& map_tile_info = world_state->getMapTileInfoForWorld(world_name, lock);

								for(size_t i=0; i<tile_coords.size(); ++i)
								{
									const Vec3<int> v(tile_coords[i].x, tile_coords[i].y, tile_coords[i].z);

									// Guardrails against abusive/accidental huge tile coordinates.
									if(v.z < 0 || v.z > 6 || std::abs(v.x) > 4096 || std::abs(v.y) > 4096)
										continue;
									...
									else
									{
										...
										info.cur_tile_screenshot->tile_z = v.z;
					...
					case Protocol::CreateChatBot: // Client wants to create a new chatbot
						{
							conPrintIfNotFuzzing("CreateChatBot");
							if(!client_user_id.valid())
							{
								writeErrorMessageToClient(socket, "You must be logged in to create a bot.");
								break;
							}
							if(world_state->isInReadOnlyMode())
							{
							...
							const std::string bot_name = msg_buffer.readStringLengthFirst(ChatBot::MAX_NAME_SIZE);
							...
							ChatBotRef chatbot = new ChatBot();
							{
								...
								chatbot->owner_id = client_user_id;
							...
							// Reply with ChatBotCreated: bot_id + avatar_uid
							...
							conPrintIfNotFuzzing("Created chatbot id=" + toString(chatbot->id));
							break;
						}
					case Protocol::UpdateChatBot: // Client wants to update chatbot settings
						{
							conPrintIfNotFuzzing("UpdateChatBot");
							if(!client_user_id.valid())
							{
								writeErrorMessageToClient(socket, "You must be logged in to update a bot.");
								break;
							}

...
В source есть условные ветви совместимости с Qt 6, но каноническая конфигурация репозитория на 2026-07-10 использует **Qt 5**: 5.15.16 на Windows и 5.15.10 на macOS/Linux (`scripts/config-lib.rb`). Активные link sections в `gui_client/CMakeLists.txt` ссылаются на Qt 5. Qt 6 следует считать незавершённой миграцией, а не поддерживаемым переключателем сборки.

Локальный `C:\programming\qt6_build.ps1` и упомянутый ниже `install_qt6_msvc.bat` не найдены. Команды далее сохранены как исторический план и требуют повторной верификации CMake/link/runtime до использования.

## Исторический план переключения на Qt 6 (не канонический build path)

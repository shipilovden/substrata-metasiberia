...
Для `origin` используется GitHub CLI credential-helper и локальный keyring
аккаунта владельца репозитория. Перед публикацией проверять `gh auth status`.
Нельзя доверять одному сообщению `Invalid username or token`: его частая
причина — просроченная переменная окружения `GITHUB_TOKEN`, которая имеет
приоритет над рабочей credential-helper-настройкой. Если `gh auth status`
указывает на недействительный переменный токен, удалить его только из
текущего процесса PowerShell и повторить проверку:
...
Токены и пароли не добавлять в эту документацию, репозиторий или историю.

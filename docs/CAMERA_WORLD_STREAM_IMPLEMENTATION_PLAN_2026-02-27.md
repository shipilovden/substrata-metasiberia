...
> **Проверка 2026-07-10:** документ начинался как план, но committed code уже содержит camera/camera-screen object types, editor/create flow, persistence/network fields, auto-linking и live render path. Предложенный ниже отдельный `WorldCameraManager.*` не создан; текущая реализация встроена в `GUIClient`. Остальные пункты нельзя считать реализованными без отдельной сверки.

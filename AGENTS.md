...
- Разделение Qt-веток обязательно: [`qt-build-policy.md`](docs/codex/qt-build-policy.md). `master` — canonical Qt 5 через `C:\programming\qt_build.ps1`; `qt6-integration` — отдельный Qt 6 workflow/build tree/output. Не смешивать Qt 5 и Qt 6 в одном CMake build tree.
